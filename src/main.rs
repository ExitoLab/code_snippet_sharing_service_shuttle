...
async fn list_snippets(
    ...
    let mut snippet_infos: Vec<SnippetInfo> = snippets
        ...
        .filter(|snippet| {
            ...
            snippet.is_public 
                && snippet.expires_at.map_or(true, |exp| now <= exp)
                && params.language.as_ref().map_or(true, |lang| &snippet.language == lang)
        })
...
async fn main() -> shuttle_axum::ShuttleAxum {
    ...
    let router = Router::new()
        ...
        .route("/snippets/{id}", get(get_snippet_full).delete(delete_snippet))
        .with_state(store);

    // Optional: read the port from the environment (useful if you want to log it)
    let port: u16 = std::env::var("PORT")
        .unwrap_or_else(|_| "8000".to_string())
        .parse()
        .expect("PORT must be a number");

    println!("Starting code-snippet-share on http://127.0.0.1:{}", port);
